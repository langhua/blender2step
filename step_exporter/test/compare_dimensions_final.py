"""
对比Blender模型与STEP导出模型的尺寸差异 - 最终版
用法: python compare_dimensions_final.py <step_file> <log_file>
"""
import re
import sys
...
def parse_log_file(log_file):
    ...
    objects = []
    for i, section in enumerate(obj_sections[1:], 1):
...
def main():
    if len(sys.argv) < 3:
        print("用法: python compare_dimensions_final.py <step_file> <log_file>")
        sys.exit(1)
    ...
    # 打印表头
    print(f"{'#':<3} {'对象名称':<35} {'检测类型':<25} {'Blender尺寸(mm)':<35} {'STEP面类型':<20} {'STEP尺寸(mm)':<25}")
    print("-" * 140)
    ...
    for i, (log_obj, step_obj) in enumerate(zip(log_objects, step_objects)):
        ...
        print(f"{i+1:<3} {obj_name:<35} {detected_type:<25} {blender_dims:<35} {step_faces:<20} {step_dims:<25}")
        print()
    ...
    for i, (log_obj, step_obj) in enumerate(zip(log_objects, step_objects)):
        ...
        # 检查圆角是否正确创建
        if log_obj['fillet_radius'] and log_obj['fillet_radius'] > 0:
            if 'TOROIDAL' not in type_counts:
                issues.append(f"对象{i+1} ({log_obj['name']}): Blender检测到圆角(F={log_obj['fillet_radius']:.2f})但STEP中没有TOROIDAL面")
            else:
                # 检查圆角半径是否匹配
                for face in step_obj['face_details']:
                    if face['surface_type'] == 'TOROIDAL' and 'minor_radius' in face['params']:
                        ...
                        if diff > 0.1:
                            issues.append(f"对象{i+1} ({log_obj['name']}): 圆角半径不匹配 - Blender={log_obj['fillet_radius']:.2f}, STEP={step_fillet:.2f}, 差异={diff:.2f}")
        
        # 检查倒角是否正确创建
        if log_obj['chamfer_size'] and log_obj['chamfer_size'] > 0:
