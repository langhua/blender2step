...
StdoutRedirectState setup_step_writer(STEPControl_Writer& writer, const char* filename, const char* step_schema, const char* unit, int advanced_brep, int enable_logging, const char* log_filename) {
    ...
    // 禁用 SEAM_CURVE 的导出，避免 FreeCAD 中显示黑色竖线
    Interface_Static::SetIVal("write.step.edge.mode", 0); // 禁用边缘模式，避免 SEAM_CURVE
    Interface_Static::SetIVal("write.step.seam", 0); // 禁用缝合曲线
    ...
    // 添加SEAM_CURVE设置的调试输出
    std::cout << "[STEP Exporter] DEBUG: write.step.edge.mode = " << Interface_Static::IVal("write.step.edge.mode") << std::endl;
    std::cout << "[STEP Exporter] DEBUG: write.step.seam = " << Interface_Static::IVal("write.step.seam") << std::endl;
    std::cout.flush();
