...
def create_rounded_box(name, width, depth, height, corner_radius, segments=8):
    ...
    print(f"Selected {selected_count} vertical edges for bevel")
...
shell = create_rounded_box("BottomShell", 100.0, 70.0, 10.0, 10.0, segments=8)
...
print(f"Shell vertex count: {len(shell.data.vertices)}")
print(f"Shell edge count: {len(shell.data.edges)}")
...
output_path = os.path.join(os.path.dirname(__file__), '..', '..', 'build', 'debug_rounded_corners.png')
