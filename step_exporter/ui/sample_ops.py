...
class STEP_EXPORTER_OT_create_cylinder_gallery(Operator):
    ...
    def modal(self, context, event):
        ...
        if self._phase == 1:
            if self._mod_idx < len(self._cylinders):
                obj = self._cylinders[self._mod_idx]
                ...
                for mod in reversed(list(obj.modifiers)):
                    try:
                        ...
                        pass
                self._mod_idx += 1
                pct = 80 + (self._mod_idx / len(self._cylinders)) * 15
                update_progress(pct, f"应用修改器: {self._mod_idx}/{len(self._cylinders)}", context)
            ...
            self.report({'INFO'}, f"Cylinder gallery created — {self._total} items")
            return {'FINISHED'}

        if self._shelf_idx >= len(m.SHELVES):
            self._cylinders = [o for o in bpy.data.objects
        ...
        if self._item_idx < len(items):
            ...
            m.add_cylinder(y, z, f"C{self._shelf_idx+1}_{name_sfx}",
